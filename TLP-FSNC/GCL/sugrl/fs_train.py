...
from torch_geometric.datasets import Planetoid, Amazon, Coauthor, CoraFull, HeterophilousGraphDataset
...
def get_args(model_name, dataset_class, dataset_name, custom_key="", yaml_path=None) -> argparse.Namespace:
    ...
    # Dataset
    parser.add_argument('--data-root', default="../graphdata", metavar='DIR', help='path to dataset')
...
def get_dataset(args, dataset_kwargs):
    path = '../../../graphdata'
    if args.dataset_name in ['Cora', 'CiteSeer', 'PubMed', 'Photo', 'Computers', 'Coauthor', 'CoraFull', 'Roman-empire']:
        ...
        elif args.dataset_name == 'Coauthor':
            dataset = Coauthor(root=path, name="CS")
        elif args.dataset_name == 'CoraFull':
            dataset =CoraFull(root=path)
        elif args.dataset_name == 'Roman-empire':
            print('Here')
            dataset = HeterophilousGraphDataset(root=path, name="Roman-empire")
        data = dataset[0]
    ...
    return data, [A_I_nomal,adj_1], [data.x], [lable, nb_feature, nb_classes, nb_nodes], id_by_class, train_class, dev_class, test_class
...
if __name__ == '__main__':
    ...
    main_args = get_args(
        ...
        # Planetoid,MyAmazon
        dataset_name=config['dataset'],  # Cora, CiteSeer, PubMed, Photo, Computers
